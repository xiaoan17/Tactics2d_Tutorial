...
def visualize_RS(use_arrows=True):
    """可视化Reeds-Shepp曲线插值器的路径规划结果
    ...
    # start_headings = [0.1 + 0.66 * 1]  # 单一起点测试用
    ...
    ax.set_title(f"Reeds-Shepp path planning visualization ({mode_text} mode)", fontsize=14)
    # 保存图像
    filename = "Tactics2d_Tutorial/results/RS_arrows.png" if use_arrows else "Tactics2d_Tutorial/results/RS_simple.png"
    plt.savefig(filename, dpi=300, bbox_inches='tight')

if __name__ == "__main__":
    # 设置mode
    mode = "arrows"  # 可选值为"arrows"或"simple"
    if mode == "arrows":
        visualize_RS(use_arrows=True)
    else:
        visualize_RS(use_arrows=False)
